...
Adapters provide uniform API for all the IO read/write methods. Available adapters are @coolio@, @eventmachine@, @io_select@ and @socket@.
...
<pre>
AMQ::SocketClient.connect
AMQ::SocketClient.connect(:port => 5672)
AMQ::SocketClient.connect("amqp://")
</pre>
...
<pre>
AMQ::SocketClient.connect do |client|
  client.queue("", 1)
end
</pre>
